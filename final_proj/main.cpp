...
#if MBED_CONF_APP_USE_TLS_SOCKET
    static constexpr size_t REMOTE_PORT = 12345; // TLS port
#else
...
public:
    ...
    void run() {
        ...
        printf("%s", address.get_ip_address());
        ...
        while(1){
            ...
            y = (float)((int)(y*10000))/10000;
            z = (float)((int)(z*10000))/10000;
            // int len = sprintf(acc_json,"{\"x\":%f,\"y\":%f,\"z\":%f,\"s\":%d}",x, y, z);
            int len = 0;
            if(y > 100000){
                printf("turn right\n");
                len = sprintf(acc_json,"%d", 0);
                turn = 1;
            }
            else if (y < -100000) {
                printf("turn left\n");
                len = sprintf(acc_json,"%d", 1);
                turn = 1;
            }
            else if(x > 100000){
                printf("turn front\n");
                len = sprintf(acc_json,"%d", 2);
                turn = 1;
            }
