...
class MyDelegate(DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data):
        if cHandle == 19:
            ...
            print(
                    f"A notification was received: pDataX: {ia[0]}, pDataY: {ia[1]}, pDataZ: {ia[2]} from cHandle#: {cHandle}"
            )
        else:
            i = int.from_bytes(data, "big")
            print(f"A notification was received: HeartRate: {i} from cHandle#: {cHandle}")
...
try:
    ...
    magnetoService = dev.getServiceByUUID(UUID(0x180F))
    for ch in magnetoService.getCharacteristics():
        print(str(ch))
    mg = dev.getCharacteristics(uuid=UUID(0x2A19))[0]
